class Matrix {
  ...
  // [MODIFIED] Упрощённая ReLU (можно заменить на LeakyReLU, sigmoid и т.п.)
  float relu(float x) {
    return max(0, x);
  }

  // [MODIFIED] mutate с constrain()
  void mutate(float mutationRate) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        if (random(1) < mutationRate) {
          matrix[i][j] += randomGaussian() / 5;
          matrix[i][j] = constrain(matrix[i][j], -1, 1);  // безопасная обрезка
        }
      }
    }
  }
